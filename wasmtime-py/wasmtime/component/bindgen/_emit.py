...
def _emit_variant(t: Type, ir: Resolve, ctx: _tagging.TaggingCtx, out: io.StringIO) -> None:
    ...
    # Emit the container with a ``TYPE_CHECKING`` block declaring each arm as
    # a class attribute (forward-referenced by string since the arm classes
    # appear later in the module). Arm classes subclass the container at
    # module scope; we re-attach them as ``Container.Arm`` attrs for ergonomic
    # access (``case Container.Arm():``). This single-class form lets
    # ``isinstance(x, Container)`` narrow to the arm class without any
    # redeclaration trickery.
    arm_names: list[tuple[str, str]] = [(c.name, _pascal(c.name)) for c in cases]

    out.write(f"\nclass {name}:\n")
    if docs:
        out.write(f'    """{_format_docstring(docs)}"""\n')
    out.write("    if TYPE_CHECKING:\n")
    for tag, arm in arm_names:
        out.write(f'        {arm}: ClassVar[type["_{name}_{arm}"]]\n')
    out.write("        _CASES: ClassVar[dict[str, type]]\n")
    out.write("        @staticmethod\n")
    out.write(f'        def lift(raw: _WitVariant) -> "{name}": ...\n')

    for case, (_tag, arm_name) in zip(cases, arm_names):
